...
function HomePage() {
  ...
  const { data: outgoinFriendReqs } = useQuery({
    queryKey: ["outgoinFriendReqs"],
    queryFn: getOutgoingFriendReqs,
  })

  const { mutate: sendRequestMutation, isPending } = useMutation({
    mutationFn: sendFriendRequest,
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["outgoinFriendReqs"] }),
  })

  useEffect(() => {
    const outgoingIds = new Set()
    if (outgoinFriendReqs && outgoinFriendReqs.length > 0) {
      outgoinFriendReqs.forEach((req) => {
        console.log(req)
            ...
            <div className='flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4'>
              <div>
                ...
                <p className='opacity-70'>
                  Discover perfect language exchange pertner based on your profile
                </p>
              </div>
            </div>
          </div>
          ...
          {loadingUsers ? (
            ...
            <div className='card bg-base-200 p-6 text-center'>
              ...
              <p className='text-base-content opacity-70'>
                Check back later for new language pertners!
              </p>
            </div>
          ) : (
                    ...
                    <div className='card-body p-5 space-y-4'>
                      <div className='flex items-center gap-3'>
                        <div className='avatar size-16 rounded-full'>
                          <img src={`https://toksy-backend.onrender.com/${user.profilePic}`} alt={user.fullName} />
                        </div>
